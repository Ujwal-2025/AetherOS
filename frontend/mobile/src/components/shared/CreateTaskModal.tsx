...
export function CreateTaskModal({ visible, onClose }: CreateTaskModalProps) {
  ...
  function resetAndClose() {
    setTitle(''); setPriority('medium'); setCategory(categories[0]?.id ?? 'work');
    setDuration(25); setRecurrence('none'); setShowAdvanced(false);
    onClose();
  }
  ...
  return (
    <>
        ...
        <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
            ...
            <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              {/* Category — from store, with manage button */}
              <View style={styles.field}>
                <View style={styles.fieldLabelRow}>
                  <AText variant="label" color="muted" uppercase style={styles.fieldLabel}>Category</AText>
                  <Pressable onPress={() => setShowManageCats(true)} style={styles.manageBtn}>
                    <Ionicons name="settings-outline" size={13} color={colors.text.faint} />
                    <AText style={styles.manageBtnText}>Manage</AText>
                  </Pressable>
                </View>
              ...
              <Pressable style={styles.advancedToggle} onPress={() => setShowAdvanced((v) => !v)}>
                <AText style={styles.advancedToggleText}>Advanced options</AText>
                <Ionicons name={showAdvanced ? 'chevron-up' : 'chevron-down'} size={14} color={colors.text.faint} />
              </Pressable>
      ...
      <ManageCategoriesModal visible={showManageCats} onClose={() => setShowManageCats(false)} />
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  manageBtnText:    { fontFamily: fontFamily.medium, fontSize: 11, color: colors.text.faint },
  ...
  advancedToggle:   { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing[2], paddingVertical: spacing[2] },
  advancedToggleText: { fontFamily: fontFamily.medium, fontSize: 13, color: colors.text.faint },
