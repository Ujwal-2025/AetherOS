...
export function LiveDemo() {
  ...
  function handleComplete(task: typeof TASKS[0]) {
    ...
    // Float XP text — random horizontal spread
  ...
  // Pie chart: each slice = 25% of circle (141.4 of 565.5 circumference at r=90)
  // We use stacked circles approach
  const pieR = 80
  ...
  return (
    <section className="section" style={{ background: 'linear-gradient(180deg, var(--bg-primary) 0%, #0a0815 50%, var(--bg-primary) 100%)' }}>
      <div className="container">
        ...
        <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', width: '100%', maxWidth: 220 }}>
              {TASKS.map(t => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 12, color: done.has(t.id) ? 'var(--text-secondary)' : 'var(--text-faint)', transition: 'color 0.3s' }}>
                    {t.category}
                  </span>
                </div>
              ))}
          ...
          {/* Right — Tasks + Timer */}
          <div style={{ flex: '1 1 320px', maxWidth: 480, display: 'flex', flexDirection: 'column', gap: 12 }}>
            {/* XP + Combo bar */}
            <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: 1, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Focus Timer</span>
              </div>
                  ...
                  <div key={mode.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <button
                      ...
                      style={{
                        ...
                        border: selectedTimer === mode.id
                          ? '1px solid rgba(183,109,255,0.60)'
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  border: '2px solid rgba(183,109,255,0.30)',
...
function TaskRow({ task, done, onComplete }: { task: typeof TASKS[0]; done: boolean; onComplete: () => void }) {
  return (
    <div style={{
      ...
      <div style={{
        fontSize: 11, fontWeight: 700, color: task.color,
      ...
      >
        {done ? <span style={{ color: task.color }}>✓</span> : null}
      </button>
    </div>
  )
}
