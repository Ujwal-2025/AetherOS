...
interface TaskState {
  ...
  completeTask: (id: string, streak: number) => number; // returns XP earned
...
export const useTaskStore = create<TaskState>((set, get) => ({
  ...
  completeTask: (id, streak) => {
    ...
    set((state) => ({
      tasks: state.tasks.map((t) =>
        t.id === id
          ? { ...t, status: 'completed', completedAt: Date.now(), xpReward: xp }
          : t
      ),
    }));
    return xp;
  ...
  todaysTasks: () => {
    ...
    return get().tasks.filter((t) => {
      ...
      // Tasks created today OR due today OR daily recurrence
      const createdToday = new Date(t.createdAt).toISOString().split('T')[0] === today;
      const dueToday     = t.dueDate
  ...
  xpEarnedToday: () => {
    return get().completedToday().reduce((sum, t) => sum + t.xpReward, 0);
  },
}));
